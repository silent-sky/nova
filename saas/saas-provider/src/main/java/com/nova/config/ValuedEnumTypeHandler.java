...
public class ValuedEnumTypeHandler<E extends Enum<E>> extends BaseTypeHandler<E> {
    ...
    public ValuedEnumTypeHandler(Class<E> type) {
        ...
        for (E e : enums) {
            Valued valuedEnum = (Valued) e;
            int value = valuedEnum.getValue();
            if (map.containsKey(value)) {
                throw new IllegalArgumentException(type.getSimpleName() + " contains duplicated value.");
            }
            map.put(value, e);
        }
    }
    ...
    public void setNonNullParameter(PreparedStatement ps, int i, E parameter, JdbcType jdbcType) throws SQLException {
        Valued valuedEnum = (Valued) parameter;
