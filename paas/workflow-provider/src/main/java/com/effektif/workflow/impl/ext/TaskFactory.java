...
public class TaskFactory implements Brewable {
    ...
    public Task completeTaskWithCondition(
            String taskId,
    ...
    public Task completeTaskWithConditionObject(
            String taskId,
            Map<String, Object> conditionMap,
            Map<String, Object> bindingMap,
            String actionType,
            ...
            if (conditionMap != null && conditionMap.size() > 0) {
                for (Map.Entry entry : conditionMap.entrySet()) {
                    message.data(
                            ...
                            ((TypedValue) entry.getValue()).getValue(),
                            ((TypedValue) entry.getValue()).getDataType());
                }
            }
            message.setTransientData(bindingMap);
            workflowEngine.send(message);
    ...
    private boolean mayComplete(Task task, String userId, String actionType) {
        ...
        if (WorkflowConstant.UserTaskType.ALL.equals(task.getTaskType())) {
            ...
            if (WorkflowConstant.Action.REJECT.equals(actionType) && (task.getAllPassType() == null || task.getAllPassType() == 0)) {
                return canComplete;
            }
    ...
    private String getCountersignatureResult(List<ApprovalOpinion> opinionList) {
        String result = WorkflowConstant.Action.AGREE;
        for (ApprovalOpinion opinion : opinionList) {
            if (WorkflowConstant.Action.REJECT.equals(opinion.getActionType())) {
                result = WorkflowConstant.Action.REJECT;
                break;
            }
        }
        return result;
